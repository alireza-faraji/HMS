...
frappe.ui.form.on('HMS Shift', {
	...
	onload: function(frm) {
		...
		if (frm.doc.__islocal === 1) {
			...
			frappe.call({
				method: 'inn.hms_module.doctype.hms_shift.hms_shift.is_there_open_shift',
				callback: (r) => {
					if (r.message === 1) {
	...
	opening: function (frm) {
		frappe.call({
			method: "inn.hms_module.doctype.hms_shift.hms_shift.get_max_opening_cash",
			callback: (r) => {
				if (r.message) {
	...
	close_shift_button: function (frm) {
		if (frm.is_dirty()) {
			...
			frappe.confirm(__("You are about to close the shift. Are you sure?"), function() {
				frappe.call({
					method: "inn.hms_module.doctype.hms_shift.hms_shift.close_shift",
					args: {
...
function populate_payment_refund(frm, shift_id) {
	frappe.call({
		method: 'inn.hms_module.doctype.hms_shift.hms_shift.populate_cr_payment',
		args: {
			shift_id: shift_id
		},
		callback: (r_payment) => {
			frappe.call({
				method: 'inn.hms_module.doctype.hms_shift.hms_shift.populate_cr_refund',
				args: {
					shift_id: shift_id
				},
				callback: (r_refund) => {
