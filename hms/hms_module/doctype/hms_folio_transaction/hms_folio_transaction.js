...
frappe.ui.form.on('HMS Folio Transaction', {
	...
	refresh: function(frm) {
		...
		if (frm.doc.parent) {
			frm.add_custom_button(__('Show Folio'), function () {
				let url = frappe.urllib.get_full_url('/desk#Form/hms%20Folio/' + frm.doc.parent);
				if (is_check_in == 'true') {
			...
			frm.add_custom_button(__('Show Reservation'), function () {
				frappe.call({
					method: 'hms.hms_module.doctype.hms_folio.hms_folio.get_reservation_id',
					args: {
						folio_id: frm.doc.parent
					},
					callback: (r) => {
						if (r.message) {
							let url = frappe.urllib.get_full_url('/desk#Form/hms%20Reservation/' + r.message);
							if (is_check_in == 'true') {
								url = url + '?is_check_in=true';
							}
							var w = window.open(url, "_self");
	...
	transaction_type: function (frm) {
		if (frm.doc.transaction_type != undefined) {
			frappe.call({
				method: 'hms.hms_module.doctype.hms_folio_transaction_type.hms_folio_transaction_type.get_accounts_from_id',
				args: {
	...
	mode_of_payment: function (frm) {
		if (frm.doc.mode_of_payment != undefined) {
			frappe.call({
				method: 'hms.hms_module.doctype.hms_folio_transaction.hms_folio_transaction.get_mode_of_payment_account',
				args: {
...
function get_filtered_transaction_type(frm) {
	frappe.call({
		method: 'hms.hms_module.doctype.hms_folio_transaction_type.hms_folio_transaction_type.get_filtered',
		args: {
			type:frm.doc.flag
		},
		callback: (r) => {
			if (r.message) {
				...
				frm.fields_dict['transaction_type'].get_query = function () {
					return {
						filters: [
							['HMS Folio Transaction Type', 'name', 'in', r.message]
						]
					}
				}
			}
