...
class HMSFolioTransactionType(Document):
	pass

@frappe.whitelist()
def get_filtered(type):
	return_list = []
	type_list = frappe.get_all('HMS Folio Transaction Type', filters=[['type', '=', type]], fields=['name'])
...
def get_accounts_from_id(id):
	doc = frappe.get_doc('HMS Folio Transaction Type', id)
	return doc.debit_account, doc.credit_account

@frappe.whitelist()
def get_transaction_type(type):
	return_list = []
	type_list = frappe.get_all('HMS Folio Transaction Type',
