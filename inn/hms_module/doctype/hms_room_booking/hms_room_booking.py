...
from inn.hms_module.doctype.hms_folio.hms_folio import update_close_by_reservation
...
def update_by_reservation(reservation_id):
	return_message = ''
	reservation_doc = frappe.get_doc('HMS Reservation', reservation_id)
	...
	if reservation_doc.status == 'Cancel':
		...
		room_booking_doc = frappe.get_doc('HMS Room Booking',
		...
		if not frappe.db.exists('HMS Room Booking',
								{'reference_type': 'HMS Reservation', 'reference_name': reservation_id}):
			...
			room_booking_doc = frappe.new_doc('HMS Room Booking')
			...
			room_booking_doc.reference_type = 'HMS Reservation'
			...
			if filters.get('room_type'):
				if filters.get('bed_type'):
					room_list = list(frappe.db.sql(
						"select name, room_type, bed_type from `tabHMS Room` where room_status = 'Vacant Ready' and room_type = %s and bed_type = %s",
						(filters.get('room_type'), filters.get('bed_type'))
					))
				else:
					room_list = list(frappe.db.sql(
						"select name, room_type, bed_type from `tabHMS Room` where room_status = 'Vacant Ready' and room_type = %s",
						(filters.get('room_type'))
					))
			else:
				room_list = list(frappe.db.sql(
					"select name, room_type, bed_type from `tabHMS Room` where room_status = 'Vacant Ready' "
				))
	else:
		if filters.get('start') and filters.get('end'):
			if filters.get('room_type'):
				if filters.get('bed_type'):
					room_list = list(frappe.db.sql(
						"select name, room_type, bed_type from `tabHMS Room` where room_type = %s and bed_type = %s",
						(filters.get('room_type'), filters.get('bed_type'))
					))
				else:
					room_list = list(frappe.db.sql(
						"select name, room_type, bed_type from `tabHMS Room` where room_type = %s",
						(filters.get('room_type'))
					))
			else:
				room_list = list(frappe.db.sql(
					"select name, room_type, bed_type from `tabHMS Room`"
				))
...
def get_room_book_list(filters):
	room_book_list = list(frappe.db.sql(
		"select irb.room_id from `tabHMS Room Booking` as irb "
...
def get_all_room_with_room_booking_status():
	return_list = []
	room_list = frappe.get_all('HMS Room', fields=['*'])
	room_booking_list = frappe.get_all('HMS Room Booking', filters={'status': ['in', ['Booked', 'Stayed']]}, fields=['*'])
...
def is_available(room_id, start, end, name):
	if name:
		result = frappe.db.sql(
			'SELECT room_id FROM `tabHMS Room Booking` '
...
def get_name_within_date_range(room_id, start, end):
	result = frappe.db.sql(
		'SELECT name FROM `tabHMS Room Booking` '
...
def get_room_booking(room_id, date):
	return frappe.db.sql(
		'SELECT name, start, end, room_availability, note FROM `tabHMS Room Booking` WHERE status = "Booked" AND room_id = %s AND %s >= start AND %s < end',
		(room_id, date, date))