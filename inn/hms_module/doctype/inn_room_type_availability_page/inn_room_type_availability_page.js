...
function search(doc) {
	if (doc.start != undefined && doc.end != undefined) {
		...
		frappe.call({
			method: 'inn.hms_module.doctype.inn_room_type.inn_room_type.get_all_room_type',
			callback: (resp) => {
				...
				resp.message.forEach( elm => {
					...
					function get_room_type_availability(date, fun) {
						...
						frappe.call({
							method: 'inn.hms_module.doctype.inn_room_type_availability_page.inn_room_type_availability_page.get_room_type_availability',
							args: {
								room_type: elm.name,
