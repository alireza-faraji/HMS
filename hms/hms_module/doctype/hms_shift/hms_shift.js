...
frappe.ui.form.on('HMS Shift', {
	...
	onload: function(frm) {
		...
		if (frm.doc.__islocal === 1) {
			...
			frappe.call({
				method: 'hms.hms_module.doctype.hms_shift.hms_shift.is_there_open_shift',
				callback: (r) => {
					if (r.message === 1) {
						frappe.set_route('List', 'HMS Shift');
	...
	print_cash_remittance_button: function (frm, cdt, cdn) {
		if (frm.doc.__unsaved) {
			...
			let w = window.open(frappe.urllib.get_full_url("/printview?"
				+"doctype="+encodeURIComponent("HMS Shift")
	...
	opening: function (frm) {
		frappe.call({
			method: "hms.hms_module.doctype.hms_shift.hms_shift.get_max_opening_cash",
			callback: (r) => {
				if (r.message) {
	...
	close_shift_button: function (frm) {
		if (frm.is_dirty()) {
			...
			frappe.confirm(__("You are about to close the shift. Are you sure?"), function() {
				frappe.call({
					method: "hms.hms_module.doctype.hms_shift.hms_shift.close_shift",
					args: {
...
frappe.ui.form.on('HMS CC Detail',{
...
function set_all_read_only() {
	...
	frappe.meta.get_docfield('HMS CC Detail', 'qty', cur_frm.docname).read_only = true;
}

function populate_payment_refund(frm, shift_id) {
	frappe.call({
		method: 'hms.hms_module.doctype.hms_shift.hms_shift.populate_cr_payment',
		args: {
			shift_id: shift_id
		},
		callback: (r_payment) => {
			frappe.call({
				method: 'hms.hms_module.doctype.hms_shift.hms_shift.populate_cr_refund',
				args: {
					shift_id: shift_id
				},
				callback: (r_refund) => {
