...
def is_there_open_shift():
	if frappe.get_all('HMS Shift', {'status': 'Open'}):
		return 1
	else:
		return 2

def get_last_closed_shift():
	d = frappe.get_all('HMS Shift', filters={'status': 'Closed'}, order_by='creation desc', limit_page_length=1)
	if d:
		return frappe.get_doc('HMS Shift', d[0].name)
	else:
		return None
...
def populate_cr_payment(shift_id):
	...
	reservation_list = frappe.get_all('HMS Reservation', filters={'status': ['in', ['Reserved', 'In House', 'Finish', 'Cancel']]}, fields=['*'])
	if shift_id:
		...
		if last_shift is None:
			...
			for reservation_item in reservation_list:
				folio_name = frappe.db.get_value('HMS Folio', {'reservation_id': reservation_item.name}, ['name'])
				folio_transaction_list = frappe.get_all('HMS Folio Transaction',
														filters={'parent': folio_name,
																 'transaction_type': ['in', list_of_payment_type],
																 'is_void': 0},
														fields=['*'])
				for folio_trx_item in folio_transaction_list:
					payment_detail_doc = frappe.new_doc('HMS CR Payment Detail')
					...
					payment_transaction_doc = frappe.new_doc('HMS CR Payment Transaction')
			...
			master_desk_folio_list = frappe.get_all('HMS Folio',
		...
		if len(frappe.get_all('HMS Shift')) > 0:
			last_shift = get_last_closed_shift()
			# Get Guest Folio Transactions type Payment that appear since last shift
			for reservation_item in reservation_list:
				folio_name = frappe.db.get_value('HMS Folio', {'reservation_id': reservation_item.name}, ['name'])
	...
	for mode_of_payment_item in mode_of_payment:
		new_payment_detail = frappe.new_doc('HMS CR Payment Detail')
...
def populate_cr_refund(shift_id):
	...
	transaction_list = []
	cr_refund = frappe.new_doc('HMS CR Refund Detail')
	cr_refund.type = 'Refund'
	cr_refund.amount = 0
	reservation_list = frappe.get_all('HMS Reservation', filters={'status': ['in', ['In House', 'Finish', 'Cancel']]},
									  fields=['*'])
		...
		if last_shift is None:
			...
			for reservation_item in reservation_list:
				...
				for folio_trx_item in folio_transaction_list:
					...
					refund_detail_doc = frappe.new_doc('HMS CR Refund Transaction')
...
def close_shift(shift_id):
	doc = frappe.get_doc('HMS Shift', shift_id)
	doc.time_out = datetime.now()
	doc.username = frappe.session.user
	doc.status = 'Closed'
	doc.save()

	if frappe.db.get_value('HMS Shift', {'name': shift_id}, ['status']) == 'Closed':
		return True
	else:
		return False
...
def get_max_opening_cash():
	return frappe.db.get_single_value('Hms Module Setting', 'max_opening')