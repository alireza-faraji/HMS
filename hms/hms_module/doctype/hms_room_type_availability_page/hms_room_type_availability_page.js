...
function search(doc) {
	if (doc.start != undefined && doc.end != undefined) {
		...
		frappe.call({
			method: 'hms.hms_module.doctype.hms_room_type.hms_room_type.get_all_room_type',
			callback: (resp) => {
				...
				resp.message.forEach( elm => {
					...
					function get_room_type_availability(date, fun) {
						...
						frappe.call({
							method: 'hms.hms_module.doctype.hms_room_type_availability_page.hms_room_type_availability_page.get_room_type_availability',
							args: {
								room_type: elm.name,
