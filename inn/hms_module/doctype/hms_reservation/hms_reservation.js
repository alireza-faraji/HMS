...
frappe.ui.form.on('HMS Reservation', {
	onload: function(frm) {
	...
	refresh: function(frm) {
		...
		if (frm.doc.__islocal !== 1) {
			frm.add_custom_button(__('Show Folio'), function () {
				frappe.call({
					method: 'inn.hms_module.doctype.hms_reservation.hms_reservation.get_folio_url',
					args: {
		...
		if (frm.doc.__islocal !== 1 && frm.doc.status === 'Reserved') {
			...
			if (is_check_in === undefined) {
				...
				frm.add_custom_button(__("Start Check In Process"), function () {
					is_check_in = "true";
					frappe.call({
						method:"inn.hms_module.doctype.hms_reservation.hms_reservation.start_check_in",
						args: {
				...
				if (is_form_not_good_to_go === true && (error_message !== '' || error_message !== 'Please fill these fields before Finishing Check In process: <br /> <ul>')) {
					...
					frm.add_custom_button(__("Finish Check In Process"), function () {
						if (frm.doc.__unsaved !== undefined || frm.doc.unsaved == 1) {
							...
							is_check_in = "false";
							frappe.call({
								method: 'inn.hms_module.doctype.hms_reservation.hms_reservation.check_in_reservation',
								args: {
									reservation_id: frm.doc.name
								},
								callback: (r) => {
									if (r.message === 'In House') {
										frappe.call({
											method: 'inn.hms_module.doctype.hms_room_booking.hms_room_booking.update_by_reservation',
											args: {
												reservation_id: frm.doc.name
											},
											callback: (r) => {
					...
					// frappe.call({
					// 	method: 'inn.hms_module.doctype.hms_reservation.hms_reservation.allowed_to_in_house',
					// 	args: {
					// 		reservation_id: frm.doc.name
					// 	},
					// 	callback: (r) => {
			...
			frm.page.add_menu_item(__('Cancel'), function () {
				frappe.confirm(__("You are about to Cancel this Reservation. Are you sure?"), function () {
					frappe.call({
						method: 'inn.hms_module.doctype.hms_reservation.hms_reservation.cancel_reservation',
						args: {
			...
			frm.page.add_menu_item(__('No Show'), function () {
				frappe.confirm(__("You are about to mark the guest as No Show. Are you Sure?"), function () {
					frappe.call({
						method: 'inn.hms_module.doctype.hms_reservation.hms_reservation.no_show_reservation',
						args: {
		...
		if (frm.doc.__islocal !== 1 && frm.doc.status === 'In House') {
			...
			frm.page.add_menu_item(__('Cancel'), function () {
				frappe.confirm(__("You are about to Cancel this Reservation with status already <b>In House</b>. Are you sure?"), function () {
					frappe.call({
						method: 'inn.hms_module.doctype.hms_reservation.hms_reservation.cancel_single_reservation_in_house',
						args: {
							reservation_id: frm.doc.name,
						},
						callback: (r) => {
	...
	after_save: function(frm) {
		frappe.call({
			method: 'inn.hms_module.doctype.hms_folio.hms_folio.create_folio',
			args: {
				reservation_id: frm.doc.name
			}
		});
	...
	expected_arrival: function(frm) {
		frappe.call({
			method: 'inn.hms_module.doctype.hms_audit_log.hms_audit_log.get_last_audit_date',
			callback: (r) => {
				if (r.message) {
			...
			callback: (r) => {
				if (r.message) {
					...
					if (frm.doc.departure < r.message) {
						...
						frappe.call({
							method: 'inn.hms_module.doctype.hms_room_booking.hms_room_booking.get_room_booking_name_by_reservation',
							args: {
								reservation_id: frm.doc.name
							},
							callback: (r) => {
								if (r.message) {
									...
									frappe.call({
										method: 'inn.hms_module.doctype.hms_room_booking.hms_room_booking.get_name_within_date_range',
										args: {
	...
	room_rate: function (frm) {
		if (frm.doc.room_rate !== undefined) {
			frappe.call({
				method: 'inn.hms_module.doctype.hms_room_rate.hms_room_rate.get_base_room_rate',
				args: {
	...
	issue_card: function (frm) {
		if (frm.doc.__islocal !== 1 && frm.doc.status === 'In House') {
			...
			if (current_active_card >= room_max_active_card) {
				...
				frappe.call({
					method: 'inn.hms_module.doctype.hms_key_card.hms_key_card.issue_card',
					args: {
						reservation_id: frm.doc.name
					},
					callback: (r) => {
	...
	verify_card: function () {
		frappe.call({
			method: 'inn.hms_module.doctype.hms_key_card.hms_key_card.verify_card',
			args: {
				track: "3"
			}
		});
	...
	test_1: function () {
		frappe.call({
			method: 'inn.hms_module.doctype.hms_key_card.hms_key_card.test_api',
			args: {
				option: "1"
			}
		});
...
frappe.ui.form.on('HMS Key Card',{
...
function autofill(frm) {
	...
	if (frm.doc.actual_room_id === undefined || frm.doc.actual_room_id == null || frm.doc.actual_room_id === '') {
		frappe.call({
			method: 'inn.hms_module.doctype.hms_room.hms_room.get_room_status',
			args: {
...
function manage_filters(fieldname, phase, start_date) {
	...
	if (fieldname === 'room_type') {
		...
		if(cur_frm.doc.actual_room_id !== undefined) {
			frappe.db.get_value('HMS Room', cur_frm.doc.actual_room_id, ['room_type', 'bed_type'], function (r) {
		...
		if (cur_frm.doc.room_id !== undefined) {
			frappe.db.get_value('HMS Room', cur_frm.doc.room_id, ['room_type', 'bed_type'], function (r) {
				cur_frm.doc.room_type = r.room_type;
				cur_frm.doc.bed_type = r.bed_type;
				cur_frm.refresh();
				get_available('room_type', phase);
...
function get_available(fieldname, phase) {
	...
	if (fieldname === 'room_id' || fieldname === 'actual_room_id') {
		query = 'inn.hms_module.doctype.hms_room_booking.hms_room_booking.get_room_available';
	}
	else if (fieldname === 'room_type') {
		query = 'inn.hms_module.doctype.hms_room_booking.hms_room_booking.get_room_type_available';
	}
	else if (fieldname === 'bed_type') {
		query = 'inn.hms_module.doctype.hms_room_booking.hms_room_booking.get_bed_type_available';
	}
...
function get_room_max_active_card() {
	frappe.call({
		method: 'inn.hms_module.doctype.hms_key_card.hms_key_card.room_max_active_card',
		callback: (r) => {
			if (r.message) {
...
function erase_card(flag, card_name) {
	...
	frappe.call({
		method: 'inn.hms_module.doctype.hms_key_card.hms_key_card.erase_card',
		args: {
			flag: flag,
			card_name: card_name,
	...
	if (room_type !== undefined) {
		frappe.db.get_value("Customer", cur_frm.doc.customer_id, "customer_group", (customer) => {
			...
			field.get_query = function () {
				return {
					filters: [
						['HMS Room Rate', 'room_type', '=', room_type],
						['HMS Room Rate', 'is_disabled', '=', 0],
						['HMS Room Rate', 'customer_group', 'in', customer_group_list],
						['HMS Room Rate', 'from_date', '<=', start_date],
						['HMS Room Rate', 'to_date', '>=', start_date],
					]
				}
			}
		});
...
function process_check_out(frm) {
	frappe.call({
		method: 'inn.hms_module.doctype.hms_folio.hms_folio.get_balance_by_reservation',
		args: {
			reservation_id: frm.doc.name
		},
		callback: (r) => {
			if (r.message !== 0) {
				...
				if (current_active_card > 0) {
					...
					frappe.call({
						method: 'inn.hms_module.doctype.hms_reservation.hms_reservation.check_out_reservation',
						args: {
							reservation_id: frm.doc.name,
						},
						callback: (r) => {
...
function move_room(frm) {
				...
				'onchange': () => {
					...
					frappe.db.get_value("Customer", frm.doc.customer_id, "customer_group", (customer) => {
						...
						d.fields_dict['mv_room_rate'].get_query = function () {
							return {
								filters: [
									['HMS Room Rate', 'room_type', '=', d.fields_dict['mv_room_type'].get_value()],
									['HMS Room Rate', 'is_disabled', '=', 0],
									['HMS Room Rate', 'customer_group', 'in', customer_group_list],
									['HMS Room Rate', 'from_date', '<=', formatDate(new Date())],
									['HMS Room Rate', 'to_date', '>=', formatDate(new Date())],
								]
							}
						}
					});
	...
	d.set_primary_action(__('Move Room'), () => {
		...
		if (good_to_go === 1) {
			...
			frappe.call({
				method: 'inn.hms_module.doctype.hms_move_room.hms_move_room.create_move_room_by_reservation',
				args: {
					reservation_id: frm.doc.name,
...
function calculate_rate_and_bill(frm) {
	frappe.call({
		method: 'inn.hms_module.doctype.hms_reservation.hms_reservation.calculate_room_bill',
		args: {
	...
	frappe.call({
		method: 'inn.hms_module.doctype.hms_room_rate.hms_room_rate.get_actual_room_rate_breakdown',
		args: {
			room_rate_id: frm.doc.room_rate,
			actual_rate: frm.doc.actual_room_rate,
		},
...
function check_membership_cards() {
	...
	d.set_primary_action(__('Check'), ()=> {
		frappe.call({
			method: 'inn.hms_module.doctype.hms_membership_card.hms_membership_card.check_card',
			args: {
