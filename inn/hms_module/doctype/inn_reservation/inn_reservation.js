...
frappe.ui.form.on('Inn Reservation', {
	...
	refresh: function(frm) {
		...
		if (frm.doc.__islocal !== 1) {
			frm.add_custom_button(__('Show Folio'), function () {
				frappe.call({
					method: 'inn.hms_module.doctype.inn_reservation.inn_reservation.get_folio_url',
					args: {
		...
		if (frm.doc.__islocal !== 1 && frm.doc.status === 'Reserved') {
			...
			if (is_check_in === undefined) {
				...
				frm.add_custom_button(__("Start Check In Process"), function () {
					is_check_in = "true";
					frappe.call({
						method:"inn.hms_module.doctype.inn_reservation.inn_reservation.start_check_in",
						args: {
				...
				if (is_form_not_good_to_go === true && (error_message !== '' || error_message !== 'Please fill these fields before Finishing Check In process: <br /> <ul>')) {
					...
					frm.add_custom_button(__("Finish Check In Process"), function () {
						if (frm.doc.__unsaved !== undefined || frm.doc.unsaved == 1) {
							...
							is_check_in = "false";
							frappe.call({
								method: 'inn.hms_module.doctype.inn_reservation.inn_reservation.check_in_reservation',
								args: {
									reservation_id: frm.doc.name
								},
								callback: (r) => {
									if (r.message === 'In House') {
										frappe.call({
											method: 'inn.hms_module.doctype.inn_room_booking.inn_room_booking.update_by_reservation',
											args: {
												reservation_id: frm.doc.name
											},
											callback: (r) => {
					...
					// frappe.call({
					// 	method: 'inn.hms_module.doctype.inn_reservation.inn_reservation.allowed_to_in_house',
					// 	args: {
					// 		reservation_id: frm.doc.name
					// 	},
					// 	callback: (r) => {
			...
			frm.page.add_menu_item(__('Cancel'), function () {
				frappe.confirm(__("You are about to Cancel this Reservation. Are you sure?"), function () {
					frappe.call({
						method: 'inn.hms_module.doctype.inn_reservation.inn_reservation.cancel_reservation',
						args: {
			...
			frm.page.add_menu_item(__('No Show'), function () {
				frappe.confirm(__("You are about to mark the guest as No Show. Are you Sure?"), function () {
					frappe.call({
						method: 'inn.hms_module.doctype.inn_reservation.inn_reservation.no_show_reservation',
						args: {
		...
		if (frm.doc.__islocal !== 1 && frm.doc.status === 'In House') {
			...
			frm.page.add_menu_item(__('Cancel'), function () {
				frappe.confirm(__("You are about to Cancel this Reservation with status already <b>In House</b>. Are you sure?"), function () {
					frappe.call({
						method: 'inn.hms_module.doctype.inn_reservation.inn_reservation.cancel_single_reservation_in_house',
						args: {
							reservation_id: frm.doc.name,
						},
						callback: (r) => {
	...
	after_save: function(frm) {
		frappe.call({
			method: 'inn.hms_module.doctype.inn_folio.inn_folio.create_folio',
			args: {
				reservation_id: frm.doc.name
			}
		});
	...
	expected_arrival: function(frm) {
		frappe.call({
			method: 'inn.hms_module.doctype.inn_audit_log.inn_audit_log.get_last_audit_date',
			callback: (r) => {
				if (r.message) {
			...
			callback: (r) => {
				if (r.message) {
					...
					if (frm.doc.departure < r.message) {
						...
						frappe.call({
							method: 'inn.hms_module.doctype.inn_room_booking.inn_room_booking.get_room_booking_name_by_reservation',
							args: {
								reservation_id: frm.doc.name
							},
							callback: (r) => {
								if (r.message) {
									...
									frappe.call({
										method: 'inn.hms_module.doctype.inn_room_booking.inn_room_booking.get_name_within_date_range',
										args: {
	...
	room_rate: function (frm) {
		if (frm.doc.room_rate !== undefined) {
			frappe.call({
				method: 'inn.hms_module.doctype.inn_room_rate.inn_room_rate.get_base_room_rate',
				args: {
	...
	issue_card: function (frm) {
		if (frm.doc.__islocal !== 1 && frm.doc.status === 'In House') {
			...
			if (current_active_card >= room_max_active_card) {
				...
				frappe.call({
					method: 'inn.hms_module.doctype.inn_key_card.inn_key_card.issue_card',
					args: {
						reservation_id: frm.doc.name
					},
					callback: (r) => {
	...
	verify_card: function () {
		frappe.call({
			method: 'inn.hms_module.doctype.inn_key_card.inn_key_card.verify_card',
			args: {
				track: "3"
			}
		});
	...
	test_1: function () {
		frappe.call({
			method: 'inn.hms_module.doctype.inn_key_card.inn_key_card.test_api',
			args: {
				option: "1"
			}
		});
...
function autofill(frm) {
	...
	if (frm.doc.actual_room_id === undefined || frm.doc.actual_room_id == null || frm.doc.actual_room_id === '') {
		frappe.call({
			method: 'inn.hms_module.doctype.inn_room.inn_room.get_room_status',
			args: {
...
function get_available(fieldname, phase) {
	...
	if (fieldname === 'room_id' || fieldname === 'actual_room_id') {
		query = 'inn.hms_module.doctype.inn_room_booking.inn_room_booking.get_room_available';
	}
	else if (fieldname === 'room_type') {
		query = 'inn.hms_module.doctype.inn_room_booking.inn_room_booking.get_room_type_available';
	}
	else if (fieldname === 'bed_type') {
		query = 'inn.hms_module.doctype.inn_room_booking.inn_room_booking.get_bed_type_available';
	}
...
function get_room_max_active_card() {
	frappe.call({
		method: 'inn.hms_module.doctype.inn_key_card.inn_key_card.room_max_active_card',
		callback: (r) => {
			if (r.message) {
...
function erase_card(flag, card_name) {
	...
	frappe.call({
		method: 'inn.hms_module.doctype.inn_key_card.inn_key_card.erase_card',
		args: {
			flag: flag,
			card_name: card_name,
...
function process_check_out(frm) {
	frappe.call({
		method: 'inn.hms_module.doctype.inn_folio.inn_folio.get_balance_by_reservation',
		args: {
			reservation_id: frm.doc.name
		},
		callback: (r) => {
			if (r.message !== 0) {
				...
				if (current_active_card > 0) {
					...
					frappe.call({
						method: 'inn.hms_module.doctype.inn_reservation.inn_reservation.check_out_reservation',
						args: {
							reservation_id: frm.doc.name,
						},
						callback: (r) => {
...
function move_room(frm) {
	...
	d.set_primary_action(__('Move Room'), () => {
		...
		if (good_to_go === 1) {
			...
			frappe.call({
				method: 'inn.hms_module.doctype.inn_move_room.inn_move_room.create_move_room_by_reservation',
				args: {
					reservation_id: frm.doc.name,
...
function calculate_rate_and_bill(frm) {
	frappe.call({
		method: 'inn.hms_module.doctype.inn_reservation.inn_reservation.calculate_room_bill',
		args: {
	...
	frappe.call({
		method: 'inn.hms_module.doctype.inn_room_rate.inn_room_rate.get_actual_room_rate_breakdown',
		args: {
			room_rate_id: frm.doc.room_rate,
			actual_rate: frm.doc.actual_room_rate,
		},
...
function check_membership_cards() {
	...
	d.set_primary_action(__('Check'), ()=> {
		frappe.call({
			method: 'inn.hms_module.doctype.inn_membership_card.inn_membership_card.check_card',
			args: {
