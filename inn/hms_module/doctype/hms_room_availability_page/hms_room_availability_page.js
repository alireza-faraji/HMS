...
function search(doc) {
	if (doc.start != undefined && doc.end != undefined) {
		...
		frappe.call({
			method: 'inn.hms_module.doctype.hms_room.hms_room.get_all_hms_room',
			callback: (resp) => {
				resp.message.forEach(elm => {
					...
					function get_availability(date, fun) {
						...
						frappe.call({
							method: 'inn.hms_module.doctype.hms_room_availability_page.hms_room_availability_page.get_room_availability',
							args: {
								room_id: elm.name,
...
function book_dialog(room_id, date, current_status) {
	if (current_status != 'Room Sold') {
		if (current_status == '') {
			...
			frappe.call({
				method: 'inn.hms_module.doctype.hms_room_booking.hms_room_booking.get_room_booking',
				args: {
					room_id: room_id,
					date: date
				},
...
function process_booking(room_id, form, flag, name) {
	frappe.call({
		method: 'inn.hms_module.doctype.hms_room_booking.hms_room_booking.is_available',
		args: {
			room_id: room_id,
			start: form.start,
			end: form.end,
