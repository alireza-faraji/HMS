...
def get_room_rate_after_tax(room_rate_id):
	doc = frappe.get_doc('HMS Room Rate', room_rate_id)
	_, _, room_rate_after_tax = calculate_hms_tax_and_charges(float(doc.room_rate), doc.room_rate_tax)
	return room_rate_after_tax[-1]


def get_breakfast_rate_after_tax(room_rate_id):
	doc = frappe.get_doc('HMS Room Rate', room_rate_id)
	_, _, breakfast_rate_after_tax = calculate_hms_tax_and_charges(float(doc.breakfast_rate), doc.breakfast_tax)
...
def get_actual_room_rate_before_from_actual_rate(room_rate_id, actual_rate):
	...
	room_rate_tax_breakdown_list = frappe.get_all('HMS Tax Breakdown',
													filters={'parent': frappe.db.get_value('HMS Room Rate',
																						   {'name': room_rate_id},
																						   ['room_rate_tax'])},
												  	order_by="idx desc",
												  	fields=['*'])
...
def get_actual_room_rate_breakdown(room_rate_id, actual_rate):
	room_rate_doc = frappe.get_doc('HMS Room Rate', room_rate_id)
...
def calculate_rate_breakdown_from_final_rate(room_rate_tax_name, breakfast_tax_name, final_room_rate_amount, final_breakfast_rate_amount):
	...
	breakfast_rate_tax_breakdown_list = frappe.get_all('HMS Tax Breakdown',
