...
def calculate_amounts(amount_per_pax, tax_id, total_pax):
	...
	_, _, total_amount_after_tax = calculate_hms_tax_and_charges(total_amount, tax_id)
	return total_amount, total_amount_after_tax[-1]

@frappe.whitelist()
def calculate_amounts_before_tax(amount_after_tax, tax_id, total_pax):
	tax_breakdown_list = frappe.get_all('HMS Tax Breakdown',
...
def get_package_list(active_flag=None):
	...
	packages = frappe.get_all('HMS Package', filters=filter, fields=['name'])
