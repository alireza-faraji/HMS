...
frappe.ui.form.on('HMS Dayend Close', {
	onload: function(frm) {
		set_audit_date(frm);
	...
	refresh: function(frm) {
		if (frm.doc.__islocal === 1) {
			frappe.call({
				method: 'inn.hms_module.doctype.hms_dayend_close.hms_dayend_close.is_there_open_dayend_close',
				callback: (r) => {
					if (r.message === 1) {
						frappe.set_route('List', 'HMS Dayend Close');
						frappe.msgprint('Cannot Create new Dayend Close, Please proceed previous Dayend Close first.');
					}
					else {
						frappe.call({
							method: 'inn.hms_module.doctype.hms_room_charge_posting.hms_room_charge_posting.is_there_closed_room_charge_posting_at',
							callback: (resp) => {
								if (resp.message === 1) {
									populate_child(frm);
								}
								else {
									frappe.set_route('List', 'HMS Room Charge Posting');
...
function populate_child(frm) {
	frappe.call({
		...
		callback: (resp) => {
			if (resp.message) {
				frappe.call({
					method: 'inn.hms_module.doctype.hms_dayend_close.hms_dayend_close.load_child',
					args: {
						date: resp.message
					},
					callback: (r) => {
						...
						frappe.call({
							method: 'inn.hms_module.doctype.hms_room_charge_posting.hms_room_charge_posting.is_there_open_room_charge_posting',
							callback: (resp) => {
								if (resp.message === 1) {
									...
									if (frm.doc.__islocal !== 1) {
										frm.add_custom_button(__('Process Dayend Close'), function () {
											frappe.confirm(__("You are about to Close the day, Are you sure?"), function() {
												frappe.call({
													method: 'inn.hms_module.doctype.hms_dayend_close.hms_dayend_close.process_dayend_close',
													args: {
														doc_id: frm.doc.name
													},
													callback: (response) => {
