...
from inn.hms_module.doctype.hms_room_rate.hms_room_rate import get_actual_room_rate_breakdown
...
def create_move_room_by_reservation(reservation_id, mv_room_type, mv_bed_type, mv_room_id, mv_reason, mv_change_rate,
	...
	move_room_doc = frappe.new_doc('HMS Move Room')
	...
	# Deactivate all HMS Key Card Issued
	...
	# Update room booking
	old_room_booking = frappe.get_doc('HMS Room Booking',
	...
	if move_room_doc.old_room_id != move_room_doc.new_room:
		...
		new_room_booking = frappe.new_doc('HMS Room Booking')
		...
		new_room_booking.reference_type = 'HMS Reservation'
		new_room_booking.reference_name = reservation_id
		new_room_booking.status = 'Stayed'
		new_room_booking.insert()

	# Update Rooms Status
	old_room = frappe.get_doc('HMS Room', move_room_doc.old_room_id)
	new_room = frappe.get_doc('HMS Room', mv_room_id)
