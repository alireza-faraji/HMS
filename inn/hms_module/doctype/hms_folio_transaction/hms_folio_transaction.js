...
frappe.ui.form.on('HMS Folio Transaction', {
	...
	refresh: function(frm) {
		...
		if (frm.doc.parent) {
			...
			frm.add_custom_button(__('Show Reservation'), function () {
				frappe.call({
					method: 'inn.hms_module.doctype.hms_folio.hms_folio.get_reservation_id',
					args: {
	...
	transaction_type: function (frm) {
		if (frm.doc.transaction_type != undefined) {
			frappe.call({
				method: 'inn.hms_module.doctype.hms_folio_transaction_type.hms_folio_transaction_type.get_accounts_from_id',
				args: {
	...
	mode_of_payment: function (frm) {
		if (frm.doc.mode_of_payment != undefined) {
			frappe.call({
				method: 'inn.hms_module.doctype.hms_folio_transaction.hms_folio_transaction.get_mode_of_payment_account',
				args: {
...
function get_filtered_transaction_type(frm) {
	frappe.call({
		method: 'inn.hms_module.doctype.hms_folio_transaction_type.hms_folio_transaction_type.get_filtered',
		args: {
			type:frm.doc.flag
		},
		callback: (r) => {
