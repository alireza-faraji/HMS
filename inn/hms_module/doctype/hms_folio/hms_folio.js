	...
	refresh: function (frm, cdt, cdn) {
		...
		if (frm.doc.__islocal !== 1) {
			...
			if (frm.doc.status === 'Open') {
				...
				frappe.call({
					method: 'inn.hms_module.doctype.hms_folio.hms_folio.need_to_update_balance',
					args: {
					...
					callback: (r) => {
						if (r.message === 1) {
							// update needed
							frappe.call({
								method: 'inn.hms_module.doctype.hms_folio.hms_folio.update_balance',
								args: {
									folio_id: frm.doc.name
								},
								callback: (r) => {
...
frappe.ui.form.on('HMS Folio Transaction', {
...
function make_read_only(frm) {
	...
	frappe.meta.get_docfield('HMS Folio Transaction', 'void_transaction', frm.doc.name).hidden = active_flag;
	frappe.meta.get_docfield('HMS Folio Transaction', 'flag', frm.doc.name).read_only = active_flag;
	frappe.meta.get_docfield('HMS Folio Transaction', 'transaction_type', frm.doc.name).read_only = active_flag;
	frappe.meta.get_docfield('HMS Folio Transaction', 'amount', frm.doc.name).read_only = active_flag;
	frappe.meta.get_docfield('HMS Folio Transaction', 'debit_account', frm.doc.name).read_only = active_flag;
	frappe.meta.get_docfield('HMS Folio Transaction', 'credit_account', frm.doc.name).read_only = active_flag;
	frappe.meta.get_docfield('HMS Folio Transaction', 'remark', frm.doc.name).read_only = active_flag;
...
function add_package(frm) {
	frappe.call({
		method: 'inn.hms_module.doctype.hms_package.hms_package.get_package_list',
		args: {
			active_flag: 1
		},
		callback: (r) => {
			...
			d.set_primary_action(__('Save'), () => {
				...
				frappe.call({
					method: 'inn.hms_module.doctype.hms_folio_transaction.hms_folio_transaction.add_package_charge',
					args: {
...
function add_charge(frm) {
	frappe.call({
		method: 'inn.hms_module.doctype.hms_folio_transaction_type.hms_folio_transaction_type.get_transaction_type',
		args: {
			type: 'Debit'
		},
		callback: (r)=> {
			...
			d.set_primary_action(__('Save'), () => {
				...
				frappe.call({
					method: 'inn.hms_module.doctype.hms_folio_transaction.hms_folio_transaction.add_charge',
					args: {
	...
	frappe.call({
		...
		callback: (r)=> {
			...
			d.set_primary_action(__('Save'), () => {
				...
				frappe.call({
					method: 'inn.hms_module.doctype.hms_folio_transaction.hms_folio_transaction.add_payment',
					args: {
						transaction_type: d.get_values().transaction_type,
						amount: d.get_values().amount,
...
function transfer_to_another_folio(frm, trx_selected) {
		...
		fields: [
			{
				...
				'get_query': function () {
					return {
						filters: [
							['HMS Folio', 'name', '!=', frm.doc.name],
							['HMS Folio', 'status', '=', 'Open'],
						]
					}
				},
				reqd: 1
	...
	d.set_primary_action(__('Transfer'), () => {
		frappe.call({
			method: 'inn.hms_module.doctype.hms_folio.hms_folio.transfer_to_another_folio',
			args: {
...
function void_transaction(child) {
	...
	if (child.is_void === 0) {
		if (!child.journal_entry_id) {
			if (child.void_id) {
				frappe.call({
					method: 'inn.hms_module.doctype.hms_void_folio_transaction.hms_void_folio_transaction.request_status',
					args: {
						id: child.void_id
					},
					callback: (r) => {
						if (r.message == 'Requested') {
							...
							frappe.call({
								method: 'inn.hms_module.doctype.hms_folio_transaction_bundle.hms_folio_transaction_bundle.get_trx_list',
								args: {
									trx_id: child.name,
									len_only: true
								},
...
function void_window(child, bundle_len) {
	...
	d.set_primary_action(__('Request Void'), () => {
		frappe.call({
			method: 'inn.hms_module.doctype.hms_folio_transaction.hms_folio_transaction.void_transaction',
			args: {
				trx_id: child.name,
...
function close_folio(frm) {
	frappe.confirm(__("You are about to Close this Folio. Are you sure?"), function () {
		frappe.call({
			method:'inn.hms_module.doctype.hms_folio.hms_folio.close_folio',
			args: {
				folio_id: frm.doc.name
			},
			callback: (r) => {
