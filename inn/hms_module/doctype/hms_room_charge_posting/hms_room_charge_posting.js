...
frappe.ui.form.on('HMS Room Charge Posting', {
	onload: function(frm) {
		set_audit_date(frm);
	...
	populate: function(frm, cdt, cdn) {
		frappe.call({
			method: 'inn.hms_module.doctype.hms_room_charge_posting.hms_room_charge_posting.populate_tobe_posted',
			callback: (r) => {
				if (r.message) {
		...
		if (frm.doc.__unsaved) {
			...
			if (trx_selected.length === 0) {
				...
				frappe.confirm(__("You are about post some the Room Charges. Are you sure?"), function () {
					frappe.call({
						method: 'inn.hms_module.doctype.hms_room_charge_posting.hms_room_charge_posting.post_individual_room_charges',
						args: {
		...
		if (frm.doc.__unsaved) {
			...
			if (frm.doc.tobe_posted.length > 0) {
				frappe.confirm(__("You are about post all of the Room Charges. Are you sure?"), function () {
					frappe.call({
						method: 'inn.hms_module.doctype.hms_room_charge_posting.hms_room_charge_posting.post_room_charges',
						args: {
							parent_id: frm.doc.name,
