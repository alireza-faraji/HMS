frappe.listview_settings['HMS Membership Card'] = {
...
function generate_card() {
    frappe.call({
        method:'inn.hms_module.doctype.hms_membership_card.hms_membership_card.get_years_to_expire',
        callback: (r) => {
           let fields = [
               {
           ...
           d.set_primary_action(__('Generate'), ()=> {
               frappe.call({
                  method: 'inn.hms_module.doctype.hms_membership_card.hms_membership_card.generate_bulk_cards',
                  args: {
                   ...
                   callback: (r) => {
                      if (r.message) {
                          frappe.set_route("List", "HMS Membership Card",{"":""});
