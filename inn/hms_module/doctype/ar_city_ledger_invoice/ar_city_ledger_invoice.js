	...
	make_payment: function (frm) {
		frappe.confirm(__("Please make sure that the payment details (<b>Payment Date, Amount and Mode of Payment</b>) are correct, and <b>Outstanding Amount is zero</b>. Are you want to continue?"), function() {
			if (frm.doc.outstanding != 0.0) {
				...
				frappe.call({
					method: "inn.hms_module.doctype.ar_city_ledger_invoice.ar_city_ledger_invoice.make_payment",
					args: {
						id: frm.doc.name,
					},
					callback: (r) => {
...
function autofill_by_folio(child) {
	if (child.folio_id !== undefined) {
		frappe.call({
			method: 'inn.hms_module.doctype.ar_city_ledger.ar_city_ledger.get_ar_city_ledger_by_folio',
			args: {
...
function autofill_payments_account(child) {
	frappe.call({
		method: 'inn.hms_module.doctype.ar_city_ledger_invoice.ar_city_ledger_invoice.get_payments_accounts',
		args: {
