	...
	refresh: function (frm, cdt, cdn) {
		...
		if (frm.doc.__islocal !== 1) {
			...
			if (frm.doc.status === 'Open') {
				...
				frappe.call({
					method: 'hms.hms_module.doctype.hms_folio.hms_folio.need_to_update_balance',
					args: {
					...
					callback: (r) => {
						if (r.message === 1) {
							// update needed
							frappe.call({
								method: 'hms.hms_module.doctype.hms_folio.hms_folio.update_balance',
								args: {
									folio_id: frm.doc.name
								},
								callback: (r) => {
			...
			if (frm.doc.reservation_id !== undefined) {
				...
				let url = frappe.urllib.get_full_url('/desk#Form/hms%20Reservation/' + frm.doc.reservation_id);
				if (is_check_in === 'true') {
...
function add_package(frm) {
	frappe.call({
		method: 'hms.hms_module.doctype.hms_package.hms_package.get_package_list',
		args: {
			active_flag: 1
		},
		callback: (r) => {
			...
			d.set_primary_action(__('Save'), () => {
				...
				frappe.call({
					method: 'hms.hms_module.doctype.hms_folio_transaction.hms_folio_transaction.add_package_charge',
					args: {
...
function add_charge(frm) {
	frappe.call({
		method: 'hms.hms_module.doctype.hms_folio_transaction_type.hms_folio_transaction_type.get_transaction_type',
		args: {
			type: 'Debit'
		},
		callback: (r)=> {
...
function transfer_to_another_folio(frm, trx_selected) {
	...
	d.set_primary_action(__('Transfer'), () => {
		frappe.call({
			method: 'hms.hms_module.doctype.hms_folio.hms_folio.transfer_to_another_folio',
			args: {
...
function void_transaction(child) {
	...
	if (child.is_void === 0) {
		if (!child.journal_entry_id) {
			if (child.void_id) {
				frappe.call({
					method: 'hms.hms_module.doctype.hms_void_folio_transaction.hms_void_folio_transaction.request_status',
					args: {
						id: child.void_id
					},
					callback: (r) => {
						if (r.message == 'Requested') {
							...
							frappe.call({
								method: 'hms.hms_module.doctype.hms_folio_transaction_bundle.hms_folio_transaction_bundle.get_trx_list',
								args: {
									trx_id: child.name,
									len_only: true
								},
...
function void_window(child, bundle_len) {
	...
	d.set_primary_action(__('Request Void'), () => {
		frappe.call({
			method: 'hms.hms_module.doctype.hms_folio_transaction.hms_folio_transaction.void_transaction',
			args: {
				trx_id: child.name,
...
function close_folio(frm) {
	frappe.confirm(__("You are about to Close this Folio. Are you sure?"), function () {
		frappe.call({
			method:'hms.hms_module.doctype.hms_folio.hms_folio.close_folio',
			args: {
				folio_id: frm.doc.name
			},
			callback: (r) => {
