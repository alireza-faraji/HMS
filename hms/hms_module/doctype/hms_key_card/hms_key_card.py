...
def room_max_active_card():
	return frappe.db.get_single_value('Hms Module Setting', 'room_max_active_card')

@frappe.whitelist()
def issue_card(reservation_id):
	door_lock_provider = frappe.db.get_single_value('Hms Module Setting', 'door_lock_api_provider')
	if door_lock_provider == 'TESA':
		...
		new_card = frappe.new_doc('HMS Key Card')
		...
		if new_card.card_number == "E2" or new_card.card_number == "ED":
			...
			new_card.parenttype = 'HMS Reservation'
			new_card.insert()
...
def erase_card(flag, card_name):
	door_lock_provider = frappe.db.get_single_value('Hms Module Setting', 'door_lock_api_provider')
	if door_lock_provider == 'TESA':
		doc = frappe.get_doc('HMS Key Card', card_name)
		room = doc.room_id
...
def tesa_erase():
	# api-endpoint
	api_checkin_url = frappe.db.get_single_value('Hms Module Setting', 'card_api_url') + '/checkin'
	...
	if api_checkin_url is not None:
		if int(frappe.db.get_single_value('Hms Module Setting', 'card_use_auth')) == 1:
			auth = (frappe.db.get_single_value('Hms Module Setting', 'card_api_user'),
					frappe.db.get_single_value('Hms Module Setting', 'card_api_password'))
		...
		frappe.msgprint("Card API url not defined yet. Define the URL in HMS Hotel Setting")
...
def tesa_verify(track):
	# api-endpoint
	api_verify_url = frappe.db.get_single_value('Hms Module Setting', 'card_api_url') + '/verify'
...
def dows_verify():
	api_checkin_url = frappe.db.get_single_value('Hms Module Setting', 'card_api_url') + '/verify'
	if api_checkin_url is not None:
		s = requests.Session()
...
def dows_erase(room_id):
	api_checkin_url = frappe.db.get_single_value('Hms Module Setting', 'card_api_url') + '/erase/' + room_id.replace('R-', '').zfill(4)
	if api_checkin_url is not None:
		s = requests.Session()
...
def tesa_check_in(cmd, room, activationDate, activationTime, expiryDate, expiryTime,
	...
	# api-endpoint
	url = frappe.db.get_single_value('Hms Module Setting', 'card_api_url')
	username = frappe.db.get_single_value('Hms Module Setting', 'card_api_user')
	password = frappe.db.get_single_value('Hms Module Setting', 'card_api_password')
	is_card_use_auth = int(frappe.db.get_single_value('Hms Module Setting', 'card_use_auth'))
